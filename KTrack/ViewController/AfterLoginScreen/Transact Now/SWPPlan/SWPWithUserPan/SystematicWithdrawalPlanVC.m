//
//  SystematicWithdrawalPlanVC.m
//  KTrack
//
//  Created by Ramakrishna.M.V on 07/06/18.
//  Copyright © 2018 narasimha. All rights reserved.
//

#import "SystematicWithdrawalPlanVC.h"

@interface SystematicWithdrawalPlanVC ()<UIPickerViewDelegate,UIPickerViewDataSource>{
    ...
    NSArray *arr_fundList, *arr_folioList, *arr_schemeList, *arr_swpOptionList, *arr_swpDayList, *arr_swpFreqList, *arr_payBankList;
...
@implementation SystematicWithdrawalPlanVC
...
-(void)applyRoundedCornerAndInitialisation{
    ...
    [txt_amount setValue:[UIColor blackColor] forKeyPath:@"_placeholderLabel.textColor"];
...
-(void)onViewInitialise{
    ...
    if (arr_primaryPan.count>0) {
        ...
        str_invName=rec_primaryPan.invName;
    }
}
    ...
    [[APIManager sharedManager]requestGetUrl:str_url parameters:nil success:^(NSDictionary *responce) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (arr_swpDayList.count>1) {
                [txt_withdrawalDay setUserInteractionEnabled:YES];
                txt_withdrawalDay.inputView=picker_dropDown;
            }
            else{
                if(arr_swpDayList.count==1){
                    [txt_withdrawalDay setUserInteractionEnabled:NO];
        ...
        if ([txt_withdrawalOption.text rangeOfString:@"Capital Appreciation" options:NSCaseInsensitiveSearch].location == NSNotFound) {
            if (txt_amount.text.length==0){
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    txt_amount.text=@"";
                    [txt_amount becomeFirstResponder];
                    [[SharedUtility sharedInstance]showAlertViewWithTitle:KTError withMessage:@"Enter amount should be greater than mimimum redemption amount."];
                });
            }
...
-(void)createDictionaryAndPassValueToSWPConfirmationScreen:(NSDictionary *)responseDic{
    ...
    if ([txt_withdrawalOption.text rangeOfString:@"Capital Appreciation" options:NSCaseInsensitiveSearch].location == NSNotFound) {
        ...
        controller.str_fromScreen=@"Transact";
