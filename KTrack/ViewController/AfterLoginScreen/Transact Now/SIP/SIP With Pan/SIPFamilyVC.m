...
//  KTrack
//
//  Created by  ramakrishna.MV on 16/07/18.
//  Copyright © 2018 narasimha. All rights reserved.
//

#import "SIPFamilyVC.h"



@interface SIPFamilyVC ()<UITextFieldDelegate,UIPickerViewDataSource,UIPickerViewDelegate,UIScrollViewDelegate,UITableViewDataSource,UITableViewDelegate>{
    /// view
    ...
    /////  text Field
    __weak IBOutlet UITextField *tf_family;
...
-(void)addElements{
    ...
    Siptype =[[NSArray alloc]initWithObjects:@"Existing Folio-SIP Registration ",nil];
    SiptypeDicArray =[[NSArray alloc]initWithObjects:@"1",nil];
    ...
    if(PanDetials.count == 1)
    {
        ...
        tf_family.userInteractionEnabled = NO;
        
        tf_family.text=[NSString stringWithFormat:@"%@",rec_fund.invName];
        selectedPan=[NSString stringWithFormat:@"%@",rec_fund.PAN];
        if(Siptype.count == 1){
            tf_SIPType.userInteractionEnabled =YES;
            tf_SIPType.text=[NSString stringWithFormat:@"%@",Siptype[0]];
            selectSIPtypeInt =[NSString stringWithFormat:@"%@",SiptypeDicArray[0]];
            selectSIPtype =[NSString stringWithFormat:@"%@",Siptype[0]];
            tf_SIPType.userInteractionEnabled = NO;
    ...
    [UITextField withoutRoundedCornerTextField:tf_family forCornerRadius:5.0f forBorderWidth:1.5f];
...
- (IBAction)Atc_Submit:(id)sender {
    ...
    if ([selectSIPtypeInt isEqualToString:@"1"]) {
        ...
        }else if(tf_scheme.text.length == 0)
        {
            ...
            if (tf_noodIns.text.length == 0) {
                [[SharedUtility sharedInstance]showAlertViewWithTitle:KTError withMessage:@"Please Enter  the No of Investment  "];
                
            }else if ( [str_installments integerValue]  >=   [tf_noodIns.text   integerValue] ){
                [[SharedUtility sharedInstance]showAlertViewWithTitle:KTError withMessage:[NSString stringWithFormat:@"Please Enter  the No of  Investment  greater Than Minimun%@ ",str_installments]];
            }
            else if ([tf_noodIns.text   integerValue]   > 500){
                [[SharedUtility sharedInstance]showAlertViewWithTitle:KTError withMessage:[NSString stringWithFormat:@"Please Enter  the No of  Investment  Less Than Minimun:500 "]];
            }
        }
            ...
            if (![btn_selectNomiee.imageView.image isEqual: [UIImage imageNamed:@"uncheck"]])
            {
                ...
                if (arr_nomieerefence.count == 0  ) {
                    ...
                    if (arr_nomieerefence.count == 1 ) {
                        ...
                        }else if ([tf_nomiee1.text floatValue ]  !=  100 ){
    ...
    else if ([selectSIPtypeInt isEqualToString:@"2"])
    {
        ...
        else if( [str_driect  isEqualToString:@"Regular"] &&  [btn_nEuin.imageView.image isEqual: [UIImage imageNamed:@"radio-on"]] && tf_euin.text.length == 0  )  {
            [[SharedUtility sharedInstance]showAlertViewWithTitle:KTError withMessage:@"Please Select  EUIN  No  "];
        ...
        else if (tf_Sipamount.text.length == 0 ||  [miniAmount integerValue]  >  [tf_Sipamount.text    integerValue] ){
        ...
        else if (tf_miniAmount.text.length == 0 ||  [lbl_SIPAmount.text integerValue]  >  [tf_miniAmount.text   integerValue] ){
...
- (IBAction)Atc_existing:(id)sender {
     ...
     tf_scheme.text= nil;
             [self->tf_category resignFirstResponder];
...
- (void)textFieldDidBeginEditing:(UITextField *)textField{
    ...
    if (txt_currentTextField==tf_family) {
        
        tf_family.inputView=picker_dropDown;
        [picker_dropDown reloadAllComponents];
    }
    ...
    if (tf_noodIns==tf_category) {
        
        tf_noodIns.inputView=picker_dropDown;
        [picker_dropDown reloadAllComponents];
        ...
        if ([selectSIPtypeInt isEqualToString:@"1"]) {
            if ([btn_exitSeheme.imageView.image isEqual: [UIImage imageNamed:@"radio-off"]]){
                ...
                str_driect =[NSString stringWithFormat:@"%@",schemmeDic.schType];
                    lbl_SIPAmount.text = [NSString stringWithFormat:@" %@",schemmeDic.minAmt];;
            }
